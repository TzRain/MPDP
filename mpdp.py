            ...
            for ddi_weight in ddi_weights:
                ...
                for hid in hiddens:
                    for epochs in grid_epochs:
                        ...
                        # training setting 
                        args.epochs = epochs
                        params = [epochs, ppi_weight, ddi_weight, hid, lr, 0]
                        ...
                        for irun in range(args.n_runs):
                            ...
                            model = MPDP(n_protein, n_drug, hid)
                            ...
                            for epoch in range(0, epochs):
                                model.train()
                                optimizer.zero_grad()
                                dti_out, ppi_out, ddi_out = model(pindex_tensor, dindex_tensor,train_tensor, ppi_edgelist,ddi_edgelist)
                                loss = criterion(dti_out, train_lbl_tensor) + args.ppi_weight * criterion2(ppi_out, ppi_edgeweight) + args.ddi_weight * criterion2(ddi_out, ddi_edgeweight)
                                loss.backward()
                                optimizer.step()
                        ...
                        performance_dict['_'.join(params[:5])] = [all_dtis, list(mean), list(std)]
                        arr = np.array(val_all_dtis)
                        print('all_dtis: ', all_dtis)
                        mean = np.mean(arr, axis=0)
                        std = np.std(arr, axis=0)
                        val_performance_dict['_'.join(params[:5])] = [val_all_dtis, list(mean), list(std)]
